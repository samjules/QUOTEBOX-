...
export async function POST(request: NextRequest) {
  ...
  let body: { businessDescription: string; businessName: string; accountId: string }
  try {
  ...
  const { businessDescription, businessName, accountId } = body
  if (!businessDescription?.trim() || !businessName?.trim() || !accountId) {
  ...
  // Generate a URL-safe slug from business name + short random suffix
...
- "image": Photo upload. Use when photos help (damage, job site, etc). Add: "imageHint": "JPG or PNG", "imageMaxMb": 5
...
Return ONLY the JSON object.`
  ...
  try {
    ...
    // Save the generated form to hosted_forms
    const formName = `${businessName} Quote Form`
    ...
    return NextResponse.json({ formId: form.id, formName, formConfig })
  } catch {
    return NextResponse.json({ error: 'AI generation failed' }, { status: 500 })
  }
}
