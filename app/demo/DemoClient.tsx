...
import LandingIosApp from './LandingIosApp'
...
function trackDemoEvent(variant: DemoVariant, event: 'view' | 'booked') {
...
export default function DemoClient({ variant }: { variant: DemoVariant }) {
  ...
  // Landing phase renders one of the two split-tested marketing pages, full-bleed —
  // both funnel into the same booking calendar below once the visitor clicks through.
  if (phase === 'landing') {
    const onStart = () => setPhase('booking')
    return variant === 'quote_form'
      ? <LandingQuoteForm onStart={onStart} />
      : <LandingIosApp onStart={onStart} />
  }
